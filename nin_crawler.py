...
# 한 페이지에서 게임 이름, 정가, 할인가를 추출하는 함수
def scrape_page(soup):
    games = []
    # 실제 HTML 구조에 맞춘 클래스 이름 (가정, 실제로 수정 필요)
    ...
    for game in game_elements:
        try:
            ...
            games.append((name, original_price, sale_price))
            print(f"추출됨: {name}, 정가: {original_price}, 할인가: {sale_price}")
...
def main():
    ...
    while True:
        try:
            ...
            # 다음 페이지 링크 찾기 (가정, 실제 클래스 이름으로 수정 필요)
    ...
    try:
        with open(full_path, "w", newline="", encoding="utf-8") as csvfile:
            ...
            writer.writerow(["Game Name", "Original Price", "Sale Price"])  # 헤더 작성
