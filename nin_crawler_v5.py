...
# 한 페이지에서 게임 이름, 정가, 할인가, 할인률, 페이지 링크를 추출하는 함수
def scrape_page(soup):
    ...
    for game in game_elements:
        try:
            ...
            # 데이터 추가 (페이지 링크 포함)
            games.append((name, original_price, sale_price, sale_rate, current_date, page_link))
            print(f"추출됨: {name}, 정가: {original_price}, 할인가: {sale_price}, 할인률: {sale_rate}, 날짜: {current_date}, 링크: {page_link}")
...
def main():
    ...
    try:
        with open(full_path, "w", newline="", encoding="utf-8") as csvfile:
            ...
            # 헤더에 Page Link 추가 (수정된 부분)
            writer.writerow(["Game Name", "Original Price", "Sale Price", "Sale Rate", "Date", "Page Link"])
