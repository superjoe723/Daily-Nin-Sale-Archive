...
# 판매 페이지의 기본 URL
base_url = "https://store.nintendo.co.kr/digital/sale"

# 다운로드 폴더 경로 (폴더)
download_folder = "crawling_results"

# 한 페이지에서 게임 이름, 정가, 할인가, 할인률을 추출하는 함수
def scrape_page(soup):
    ...
    for game in game_elements:
        try:
            ...
            # 데이터 추가
            games.append((name, original_price, sale_price, sale_rate, current_date))
            print(f"추출됨: {name}, 정가: {original_price}, 할인가: {sale_price}, 할인률: {sale_rate}, 날짜: {current_date}")
...
def main():
    ...
    try:
        with open(full_path, "w", newline="", encoding="utf-8") as csvfile:
            ...
            writer.writerow(["Game Name", "Original Price", "Sale Price", "Sale Rate", "Date"])  # 헤더에 Sale Rate 추가
